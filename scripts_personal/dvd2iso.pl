...
# Grateful thanks to gracious contributers:
...
my %AspectRatio;
my $AVSync = 0;
my $OptsOK = getopts("a:b:i:f:V:v:w:t:o:z:pehkcdDsxqr", \%Options);
...
if (!$OptsOK || $Options{h}) {
  ...
  print("     -V Set the name of the DVD volume.       \n");
...
sub MakeImage {
  ...
  for (my $Num = 0; $Num <= 9999; $Num++) { 
	...
	RunThis("mkisofs -udf -dvd-video -V $DVDVolume -o $Name $WorkDir/iso/");
...
sub MakeImageDir {
  ...
  RunThis("chmod -Rf 755 $CurrentDir");
  if (-e $CurrentDir) { RunThis("rm -rf $CurrentDir"); }
  ...
  #RunThis("dvddirgen -o $WorkDir/iso");
